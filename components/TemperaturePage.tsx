...
/**
 * Detailed conversion formula and calculation component
 */
...
/**
 * Temperature conversion table component
 */
...
/**
 * Related temperature recommendations component (flat layout)
 */
...
const HealthAlert: React.FC<{ celsius: number; t: TFunction }> = ({ celsius, t }) => {
  const { message, color, icon } = useMemo(() => {
    if (celsius >= 38) return { message: t('health.highFever'), color: '#e74c3c', icon: '🌡️' };
    if (celsius >= 37.5) return { message: t('health.lowFever'), color: '#f39c12', icon: '🌡️' };
    if (celsius < 36) return { message: t('health.hypothermia'), color: '#3498db', icon: '🌡️' };
...
/**
 * Main temperature page component
 */
export const TemperaturePage: React.FC<TemperaturePageProps> = ({
  ...
  const { formattedFahrenheit, pageUrl, structuredData } = useMemo(() => {
    ...
    return {
      ...
      pageUrl: url,
      structuredData: {
        // SEO strategy: use WebPage schema for tool pages
        webPage: {
      ...
      <div dir={locale === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <main id="main-content" className="container">
          ...
          <div className="temperature-content-grid">
            ...
            {/* Granular context insight (disabled per user request) */}
