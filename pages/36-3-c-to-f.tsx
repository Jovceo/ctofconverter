...
export const getStaticProps: GetStaticProps = async ({ locale = 'en' }) => {
    const lastUpdatedIso = getLatestModifiedDate([
        'pages/36-3-c-to-f.tsx',
        'pages/temperature-template.tsx',
        `locales/${locale}/36-3-c-to-f.json`,
    ...
    // 加载翻译数据
    const enTrans = loadJSON('en', '36-3-c-to-f.json');
    const locTrans = locale !== 'en' ? loadJSON(locale, '36-3-c-to-f.json') : {};
...
export default function Temperature36_3C({ lastUpdatedIso, pageTrans, availablePages }: {
    ...
    const { locale } = useTranslation('36-3-c-to-f');
                ...
                <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                    ...
                    <tbody>
                        ...
                        <tr ${row.celsius === celsius + '°C' ? 'style="background-color: #e8f5e8;"' : ''}>
                            <td style="padding: 10px; border-bottom: 1px solid #eee;">${row.celsius === celsius + '°C' ? '<strong>' : ''}${replacePlaceholders(row.celsius || '', {fahrenheit: formattedF})}${row.celsius === celsius + '°C' ? '</strong>' : ''}</td>
                            <td style="padding: 10px; border-bottom: 1px solid #eee;">${row.celsius === celsius + '°C' ? '<strong>' : ''}${replacePlaceholders(row.fahrenheit || '', {fahrenheit: formattedF})}${row.celsius === celsius + '°C' ? '</strong>' : ''}</td>
                            <td style="padding: 10px; border-bottom: 1px solid #eee;">${row.celsius === celsius + '°C' ? '<strong>' : ''}${replacePlaceholders(row.assessment || '', {fahrenheit: formattedF})}${row.celsius === celsius + '°C' ? '</strong>' : ''}</td>
                        </tr>
                        `).join('') : ''}
                    </tbody>
                </table>
